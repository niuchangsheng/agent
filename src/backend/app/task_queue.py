...
class TaskQueue:
    ...
    def __init__(self, max_concurrent: int = 2):
        ...
        self.completed: List[int] = []  # 已完成任务 ID
        self.cancelled: List[int] = []  # 已取消任务 ID
        self._lock = asyncio.Lock()

    async def enqueue(self, task_id: int, project_id: int, raw_objective: str) -> int:
        """添加任务到队列，返回队列位置"""
...
# 全局队列实例
global_queue = TaskQueue(max_concurrent=2)
