...
| v1.3 | 🔄 规划中 | 2026-04-16 ~ ? | 前端完善 + Docker 沙箱 |
...
- [ ] Sprint 16: Docker 沙箱隔离 — 实现 Feature 17
  ...
  - 验收：危险命令隔离、OOM 限制、降级回 subprocess

## 整体项目竣工结项标准 (Definition of Done)

### v1.3 DoD (待完成)
- [ ] Sprint 14-16 全部 `/qa` 通过
- [ ] 前端监控仪表盘显示全部指标
- [ ] ETA 显示组件集成到任务列表
- [ ] 优先级选择器支持 0-10 范围
- [ ] Docker 沙箱正确隔离危险命令
- [ ] Docker 不可用时降级回 subprocess

## 技术债务与风险追踪

| 风险项 | 等级 | 当前缓解措施 | 长期解决方案 | 状态 |
|--------|------|--------------|--------------|------|
| 沙箱非 Docker 隔离 | 高 | 临时环境变量包裹 | v1.3 Sprint 16 Docker 容器 | v1.3 待处理 |
| 前端监控仪表盘缺失 | 中 | 后端 API 已完成 | v1.3 Sprint 14 实现 | v1.3 待处理 |
| ETA 显示组件缺失 | 低 | 后端 API 已完成 | v1.3 Sprint 15 实现 | v1.3 待处理 |
| 优先级选择器缺失 | 低 | 后端 API 已完成 | v1.3 Sprint 15 实现 | v1.3 待处理 |
