...
- [ ] Glassmorphism 风格一致性
- [ ] 前端单元测试覆盖（8 tests）
- [ ] 用户旅程闭环验证：打开页面 → 提交任务 → 观察队列 → 查看执行 → 审查回放

**Sprint 17: Docker 运维增强 — 实现 Feature 18 + Feature 19 + Feature 20**
- [/] Docker 沙箱配置管理 UI、配置持久化、合法性校验 — 修复完成，待 QA 复审
- [/] 容器资源监控卡片、CPU/内存阈值告警、历史容器统计 — 修复完成，待 QA 复审
- [/] Docker 日志聚合视图、分页加载、级别筛选、流式追加 — 修复完成，待 QA 复审
