...
1. 读取 `.claude/agents/planner.md` 获取作为产品架构师的深度指导原则。
...
5. 将模块功能拆解为独立的 Feature（并为每个标注复杂度、MVP 必要性、风险与降级方案）。将这些 Feature 组织成有明确先后顺序的 Sprint 周期分解，并在规范文件的末尾附上整个项目的最终验收标准（DoD）。
